...
address');
      ...
      if (!validatePassword(password)) {
        Alert.alert('Invalid Password', 'Password must be at least 8 
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result.type === 'success') {
        const credential = 
firebase.auth.GoogleAuthProvider.credential(result.params.id_token);
  ...
  const handleFacebookSignIn = async () => {
    try {
      ...
      if (result.type === 'success') {
        const credential = 
firebase.auth.FacebookAuthProvider.credential(result.params.access_token);
        await firebase.auth().signInWithCredential(credential);
        navigation.replace('Home');
      }
    } catch (error) {
  ...
  return (
    <LinearGradient colors={[colors.green, colors.gold]} 
style={styles.container}>
      ...
      <View style={styles.socialContainer}>
        ...
        >
          <Text style={styles.socialButtonText}>Sign up with 
Facebook</Text>
        </TouchableOpacity>
      </View>
